...
type PreviewState = { front: 0 | 1; committedDoc: string };
type PreviewAction = { type: "promote"; front: 0 | 1; committedDoc: string };
...
function previewReducer(
  ...
  return {
    front: action.front,
    committedDoc: action.committedDoc,
  };
}
...
export function MockPreviewFrame({ srcDoc }: { srcDoc: string }) {
  ...
  const docFor = (idx: 0 | 1) =>
    idx === preview.front ? preview.committedDoc : srcDoc;

  const handleLoad = (idx: 0 | 1) => () => {
    if (idx === preview.front) return; // 表側のロード（初期表示など）は入れ替え不要
    if (srcDoc === preview.committedDoc) return; // 表に出ている内容と同じなら入れ替え不要
    // 裏に最新が描画し終わった → 確定して表へ昇格（＝表裏を入れ替え）
    dispatchPreview({ type: "promote", front: idx, committedDoc: srcDoc });
  };
  ...
  return (
    ...
    <div className="relative h-full" style={{ minWidth: MOCK_PREVIEW_MIN_WIDTH }}>
      {([0, 1] as const).map((i) => (
        <iframe
          key={i}
          title={i === preview.front ? "mock preview" : "mock preview (buffer)"}
          srcDoc={docFor(i)}
          onLoad={handleLoad(i)}
          aria-hidden={i !== preview.front}
          className="absolute inset-0 w-full h-full bg-white block"
          style={{
            opacity: i === preview.front ? 1 : 0,
            zIndex: i === preview.front ? 1 : 0,
            pointerEvents: i === preview.front ? "auto" : "none",
          }}
